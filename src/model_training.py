...
try:
    ...
    print(f"Error loading configuration, raising exception.")
    raise e


def load_company_dataframe(company: str) -> tuple[pl.DataFrame, list[str]]:
    lazyframe = pl.scan_parquet(
        os.path.join(MODELED_PARQUET_DIR, f"{company}_share_prices_modeled.parquet")
    )
...
def make_train_test_split(X, y, test_fraction: float = 0.20):
...
def train_classification_model(X, y) -> tuple[object, dict]:
...
def train_regression_model(X, y) -> tuple[object, dict]:
    X_train, X_test, y_train, y_test = make_train_test_split(X, y)
    time_series_split = make_time_series_split(X_train)
...
def main():
    ...
    for company in COMPANIES:
        ...
        company_dataframe, feature_columns = load_company_dataframe(company)
        ...
        print(f"\nFinished model training for {company}:")
